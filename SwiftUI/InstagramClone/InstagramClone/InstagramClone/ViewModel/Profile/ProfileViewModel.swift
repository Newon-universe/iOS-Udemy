...
class ProfileViewModel: ObservableObject {
    ...
    func fetchUserStats() {
        ...
        COLLECTION_FOLLWING.document(uid).collection("user-following").getDocuments { snapshot, _ in
            guard let following = snapshot?.documents.count else { return }
            
            COLLECTION_FOLLWERS.document(uid).collection("user-followers").getDocuments { snapshot, _ in
