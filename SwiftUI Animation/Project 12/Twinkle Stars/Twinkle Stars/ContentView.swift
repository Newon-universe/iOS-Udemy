    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                CreateStar(starPoints: 5, starThickness: $starThickness, animateStar: $animateBlue, fillColor: .blue, starSizeStart: 0.1, starSizeEnd: 0.3, shadowColor: .blue)
                    .offset(x: 70, y: -60)
                    .onAppear {
                        withAnimation(.easeOut(duration: 3.3).repeatForever(autoreverses: true)) {
                            animateBlue.toggle()
                            starThickness = 0.35
                        }
                    }
                
                CreateStar(starPoints: 6, starThickness: $starThickness, animateStar: $animateRed, fillColor: .red, starSizeStart: 0.1, starSizeEnd: 0.3, shadowColor: .red)
                    .offset(x: -70, y: -150)
                    .onAppear {
                        withAnimation(.easeOut(duration: 3.3).repeatForever(autoreverses: true)) {
                            animateRed.toggle()
                ...
                CreateStar(starPoints: 6, starThickness: $starThickness, animateStar: $animatePurple, fillColor: .purple, starSizeStart: 0.6, starSizeEnd: 0.2, shadowColor: .purple)
                    .offset(x: 80, y: -120)
                    .onAppear {
                        withAnimation(.easeOut(duration: 3.3).repeatForever(autoreverses: true)) {
                ...
                CreateStar(starPoints: 6, starThickness: $starThickness, animateStar: $animateGreen, fillColor: .green, starSizeStart: 0.35, starSizeEnd: 0.6, shadowColor: .green)
                    .offset(x: -80, y: -170)
                    .onAppear {
                        withAnimation(.easeOut(duration: 5.3).repeatForever(autoreverses: true)) {
                            animateGreen.toggle()
                            starThickness = 0.35
                        }
                    }
...
struct Star: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        // loop all the points and figure out the location of this point with some math
        for corner in 0 ..< starPoints * 2 {
            ...
            if corner.isMultiple(of: 2) {
                ...
                // in the else, we're not a multiple of 2, so we are drawint an inner point
            ...
            //check the bottom var, if this new botom point is our lowest, store it in bottomEdge for later
        ...
        // calculate how much unsed space we have at the bottom of our drawing rectangle.
